...
# Minor explanation on logistic regression

## Formulas and definitions

<br>

Let's use as example our aforementioned response variable $dec$, and let's suppose we'll use as our sole predictor $like$. In the end it all boils down to a  conditional probability for a certain value of $dec$ and $like$, let's use $dec = 1$ (p1 wants to see p2 again) for the sake of the example:
...
$$\large P(y\ | \ x)=\frac{e^{b_{0}+b_{1}x}}{1 + e^{b_{0}+b_{1}x}}, \; \normalsize where: \ y = dec;\ x=like;$$
...
$$\large \frac{P(y\ | \ x)}{1 - P(y \ | \ x)} =e^{b_{0}+b_{1}x_{1}}, \; \normalsize where: \ y = dec;\ x_{1}=like;$$
...
$$\large log(\frac{P(y\ | \ x)}{1 - P(y\ | \ x)}) =b_{0}+b_{1}x_{1}, \; \normalsize where: \ y = dec;\ x_{1}=like;$$
...
```{r}
## fun + attr + shar + sinc + prob + intel + amb + sinc * intel + amb * intel
data %>%
  na.omit(dec,like, fun, attr, shar, sinc, prob, amb) -> data
  
glm(dec ~ like + like * fun + like * attr + fun + attr + shar + sinc + prob + amb,
      data = data, 
      family = "binomial") -> bm
